...
class Controller_Admin_Mail extends Controller_Admin
{
	public function action_index()
	{
		...
		$this->template->set_global('array', Arr::assoc_to_keyval(Model_Group::find('all'), 'id', 'name'), false);
		...
		$listings_title = array(
			'listings_breadcrumb'   =>'<li>'
									...
									. '</li>'
									. '<li>'
									. Kora\Permissions::get_link('admin/accounts', '', '<i class="fa fa-user-secret fa-fw"></i> Account', 'index')
									. '</li>',		
			'listings_title' 		=> '<i class="fa fa-inbox fa-fw"></i> Mailbox',
			'listings_extra' 		=> '',
		);
		$listings_panel_group = array(
			...
			'panel_title' 		=> '<i class="fa fa-inbox fa-fw"></i> Mailbox',
			'panel_title_ext' 	=> '',
			'panel_body' 		=> render('admin/mail/_listings'),
		);
		...
		$this->template->content = View::forge('admin/mail/index');
	}
